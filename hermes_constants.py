"""Shared constants for IVERI AI Agent (forked from Hermes Agent).
...
def sudo_invoker_default_home() -> Path | None:
    ...
    try:
        return Path(pwd.getpwnam(sudo_user).pw_dir) / ".iveri"
...
def get_hermes_home() -> Path:
    """IVERI home: context-local override → ``IVERI_HOME`` / ``HERMES_HOME`` env var → platform default."""
    override = get_hermes_home_override()
    if override:
        return Path(override)
    # Support both IVERI_HOME (preferred) and HERMES_HOME (backward compat)
    iveri_home = os.environ.get("IVERI_HOME", "").strip()
    if iveri_home:
        return Path(iveri_home)
