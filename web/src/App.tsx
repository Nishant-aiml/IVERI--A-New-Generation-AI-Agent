...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'iveri update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "iveri update";
    return (
      t.status.updateHermesConfirmMessage ??
      `This will run 'iveri update' (${cmd}) and restart the gateway when it finishes.`
    );
      ...
      description={
        sharedGateway
          ...
          : (t.status.restartGatewayConfirmMessage ??
            "This restarts the IVERI gateway process. Connected channels and active sessions will reconnect afterward.")
      }
