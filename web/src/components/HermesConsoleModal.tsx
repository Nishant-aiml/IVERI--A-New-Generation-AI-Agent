...
export function HermesConsoleModal({ open, onClose }: HermesConsoleModalProps) {
  ...
  const promptRef = useRef("iveri> ");
  const inputPromptRef = useRef("iveri> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "iveri> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to IVERI Console...\x1b[0m");
